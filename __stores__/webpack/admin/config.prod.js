...
const BUILD_DIR = process.env.BUILD_DIR || path.resolve(ROOT_DIR, './dist/admin');
...
const config = {
    entry: {
        ...
        vendors: ['react', 'react-dom', 'redux',
            'moment', 'lodash', 'react-paginate', 'react-loading', 'graphql', 'graphql-relay']
    },
    output: {
        path: BUILD_DIR,
    ...
    plugins: [
        ...
        new webpack.LoaderOptionsPlugin({
            options: {
                sassLoader: {
                    includePaths: [path.join(APP_DIR, './styles')]
                }
            }
        }),
